const releaseDiagnosisCodes = new Set([
  "release.rejected",
  "release.not_found",
  "release.conflict",
  "release.unavailable",
...
/** Retains only the frozen release/verification diagnosis vocabulary. */
export function normalizeReleaseDiagnosisCode(value: unknown): string {
  return typeof value === "string" && releaseDiagnosisCodes.has(value)
    ? value
    : "verification.failed";
}
