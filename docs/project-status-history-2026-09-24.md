# Archived delivery status snapshots through September 24, 2026

Historical record only. Statements labelled current below belong to their original checkpoints and are superseded by [the current delivery status](project-status.md) and the active PM ledger. The body is preserved in full; relative evidence links retain their original base directory.
...
Its actual product acceptance remains pending. Event Registration has accepted
ADR-0086 and an active exact-contract implementation task; Sales Pipeline remains
a candidate. These
...
Customer Requests catalogue registration exposed an obsolete Graph test that
classified every definition as another family. The bounded correction is pushed
at `3d9c101c`: 984 Graph tests, 19 regression/index tests and all nine definition
lane steps pass. Replacement [CI run 35995761289](https://github.com/ZP151/archeform/actions/runs/35995761289)
passes both Node 22.11.0 and 22.x jobs at that exact revision. The [correction evidence](acceptance/evidence/customer-requests/task4-ci-correction.md)
preserves the failed run and the original source acceptance.

The [Event Registration plan](superpowers/plans/2026-09-24-event-registration.md)
includes phone reservation, desktop roster/check-in, whole-event cancellation and
retained corrections. Source implementation starts under the exact accepted ADR;
actual product and hosted counts do not change. Thirteen current physical
definition outputs have a protected pre-Event baseline.
