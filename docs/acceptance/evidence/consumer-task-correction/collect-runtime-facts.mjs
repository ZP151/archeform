...
const projects = [
  "factory-task-correction-20260913",
  project,
  ...result.verificationRuns.map(
    (run) => `factory-preview-preview-${run.verificationRunId}`,
  ),
  ...result.previewRuns.map((run) => run.composeProjectName),
];
if (projects.some((name) => !/^factory-[a-z0-9-]+$/.test(name)))
  throw new Error("Invalid exact project identity");
writeFileSync(
  "docs/acceptance/evidence/consumer-task-correction/runtime-facts.json",
  JSON.stringify(
    {
...
console.log(
  `Recorded ${result.verificationRuns.length} bounded verification runs, ${result.previewRuns.length} Preview runs and ${new Set(projects).size} exact project identities.`,
);
