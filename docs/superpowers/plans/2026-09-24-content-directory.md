...
- Keep current catalogue limits, public compiler exports and package versions.
...
- [ ] RED: validate one canonical Knowledge Resource Directory and reject unsupported
      contacts, submissions, arbitrary URLs, uploads, private identity and extra rules.
- [ ] GREEN: add the fixed family and definition with correct checksum, guidance,
      primary job, success/correction/failure journeys and existing lock bindings.
- [ ] Prove rough directory intent routes to the correct definition in deterministic
      fixtures and unsupported needs remain explicit. Do not claim real-model accuracy.
- [ ] Run `pnpm --filter @factory/adapters test -- content-directory-definition.test.ts product-definition-data.test.ts requirement-interpreter.test.ts`.
- [ ] Register the executable case binding when its actual file exists; do not
...
- [ ] RED: full workspace supports reader search/detail and curator edit/visibility;
      switching role clears prior data and ignores outstanding prior-role responses.
- [ ] GREEN: compose existing primitives, local icons and states. Use useful
...
- [ ] Render full workspace at 390/768/1440, with long content, meaningful populated
      rows, no results and absent decorative media; check real computed geometry.
- [ ] Adapt actual bounded verifier probes to the directory list envelope without
      changing previous-family response assumptions. Execute emitted runtime probes.
- [ ] Run `pnpm --filter @factory/compiler test -- content-directory-presentation.test.ts content-directory-runtime.test.ts definition-data-compatibility.test.ts` plus affected worker tests.
...
- [ ] Run the short lane before expensive image construction; assemble actual
      immutable Published input, compile, verify and start the owned local app.
- [ ] Run `pnpm exec playwright test e2e/content-directory.spec.ts --workers=1`:
...
- [ ] Inspect actual generated screenshots for both scenario roles and their
...
- [ ] Record exact source/lock/Compilation identity, failures, ready time, cleanup,
      and required shared-contract review evidence. Reuse unaffected evidence.
- [ ] Only after all applicable outcomes pass, increment accepted definitions and
