...
| Source baseline | Established locally at root commit `d14b41dec8dd5009e1c7393e76b540ec7522a71b`; tracked project text is pinned to LF by `.gitattributes`. No remote is configured. |
| Promotion decision | The repository-baseline prerequisite is satisfied. Non-legacy candidate-to-Golden promotion remains blocked until TR-04/TR-05 supply valid trust records, policy/lifecycle decisions, and an explicit promotion decision. This record does not authorize a remote, account, publication, or external intake. |
...
4. The initial local Git baseline is explicit and verifiable. A non-legacy
   package still cannot be promoted until its exact source revision, trust
   evidence, policy/lifecycle result, and explicit promotion decision exist.
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Governance and source-baseline gate | PM | accepted | ADR-004 accepted | This ledger; `docs/project-status.md` | ADR, scope, delegated acceptance, frozen-v1 rule, and the initial Git baseline are recorded. |
...
| TR-04 | Trusted Registry lifecycle and migration | Integration | planned | TR-03 | `apps/api/trusted_registry.py`; `packages/trust/**`; `tests/api/test_trusted_registry.py` | Append-only records/revocations work; legacy is explicit; missing per-package evidence and lifecycle policy block promotion. |
...
- Task review is read-only. P0/P1 returns work to the same Integration writer;
  ...
  the changed scope.
- The root commit is a repository baseline, not per-package provenance. Missing
  exact source/evidence/lifecycle data continues to block promotion without
  blocking legacy inspection, fixture validation, or implementation.
...
- **2026-07-26:** The Controller established local root commit
  `d14b41dec8dd5009e1c7393e76b540ec7522a71b` as the initial source baseline
  after staged secret/ignore/EOL review and full QA. It has no remote. This
  clears only the repository-baseline prerequisite, not any trust-evidence or
  candidate-to-Golden promotion gate.
