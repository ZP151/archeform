...
under the founder's September 1 standing independent-review authorization.
Separate reviewer `work_orders_integrated_review` returns
`APPROVED_FOR_STANDING_ACCEPTANCE: yes`, P0/P1/P2 0/0/0, after reading current
governance/threat-model authorities and checking actual identity/transaction/UI
seams and all six physical locks. Root independently verifies the exact ADR hash.
Decision receipts: `generated/.customer-requests-decision-review/review.md/json`.
The proposed ADR bytes remain unchanged; acceptance is this separate event.

The [Customer Requests plan](../plans/2026-09-24-customer-requests.md) sequences
four source/product slices. PM assigns one strongest-model serialized Task 1
owner for only its enumerated Graph witness/schema/export files and tests,
capability composer/composition test, compiler private contract/minimal facade
and exact export test. Capture all twelve historical physical definitions before
editing and preserve generated bytes. No runtime/presentation/worker/adapter/
Workbench changes yet. Root owns CI correction, plans and PM evidence; paths are
disjoint. Freeze concrete profile/witness interfaces before the next writer.
Source implementation grants no database/service/actual acceptance/cleanup,
Product Publish, release, external provider or cloud operation. Delivered counts
remain unchanged; the unbudgeted Goal continues.

### Minimum Node CI module-loading correction — 2026-09-24

CI run `35973664121` finishes: Node 22.x passes; Node 22.11.0 fails
only when the new Playwright helper selection requires built ESM packages from
CommonJS tests. Root reproduces that failure locally with
`NODE_OPTIONS=--no-experimental-require-module`. Root owns the test-only
`e2e/package.json` ESM scope, Playwright comment, CI discovery step and the
seven affected E2E fixture/helper/spec import paths. Existing built public
entry points replace bare root aliases and direct adapter source imports;
production package types, dependencies and supported versions stay unchanged.
The first full discovery check exposes one additional consumer-approval source
import; isolated discovery confirms every other spec loads. Preserve this RED
result and verify all discovery plus the eighteen pure helpers before review.
No actual application, service, database or deployment run is included.

Root verifies the correction under the same strict Node flag: all 90 cases in
36 files are discovered; all 18 pure helpers pass. Tracked consumer no-emit
types and exact changed-file formatting pass. Existing QA owner
`work_orders_integrated_qa` performs one ordinary independent scoped review,
not a repeated full QA/release chain. A separate existing reviewer evaluates
the proposed ADR-0084 against standing decision authority; production work waits
for its exact-hash decision. These reviews own disjoint evidence paths only.

The ordinary independent correction review returns P0/P1/P2 0/0/0 and verifies
all ten final file identities. Root checks these identities again before staging.
PM accepts only this test-loading correction and authorizes its bounded branch
commit/push plus the separate accepted decision/plan documentation checkpoint.
Controller commits the correction as `45d29355`; no production source is included.
Remote minimum-version verification remains required; the local strict flag is
a reproduction aid, not an actual Node 22.11 execution. Portable result:
`docs/acceptance/evidence/consumer-source-checkpoint/e2e-module-correction.md`.

### Source checkpoint pushed; next customer-request decision assigned — 2026-09-24

Controller commits and pushes `22852e923c313d071266769bf9810937aad47017`;
local/remote tips match and the worktree is clean at handoff. CI run
`35973664121` targets that exact revision and is running in both Node jobs.
No PR, main integration, release or deployment is created. The source checkpoint
does not satisfy pending actual application acceptance.

PM assigns Tech Lead `customer_requests_decision` only proposed
`docs/adr/adr-0084-customer-requests-family.md` for roadmap wave D. It must compare
current reusable Graph, identity, transaction and UI seams against a customer
submission/reply/resolution/reopen loop with ownership, correction and recovery.
The role-profile invocation fails because it requests unavailable Spark; an
explicit strongest-model default agent handles this Graph/security proposal.
Root owns public product-fit research and PM records. No production implementation,
new contract acceptance, provider action or retry of blocked startup is authorized.
...
### 2026-09-24 Family expansion and delivery Goal opened

The founder asks to continue comprehensive family expansion through an unbudgeted
Goal and specifies mobile or web according to actual use, including continuous
deployment and delivery. The Goal is active with no token budget. Existing local
evidence is the baseline; no new family, hosted environment or release is claimed.

Root continues in the existing isolated `codex/definition-regression-entry`
worktree at `62c53884897a7f72c3a4c9fe25c2b010491515fd`. The dirty root checkout
contains independent Eval V2 work and is outside this write scope.

Ownership for this wave:

- Root: current roadmap/status/ledger, delivery-gap assessment, integration and
  verification. Package A's already accepted plan authorizes the bounded tooling
  continuation below; new-family contracts require the recorded technology gate.
- `content_directory_decision`: Tech Lead investigation and only proposed
  `docs/adr/adr-0074-content-directory-family.md`; no implementation authority.
- `definition_lane_completion`: only `scripts/regression.mjs` and its tests;
  add existing emitted numeric/calculated-control and complete-workspace cases
  to the ordinary definition lane. No generated output, dependencies, catalogue,
  runtime, API or historical fixtures may change.

The frozen tooling boundary is the existing lane CLI and summary shape. Existing
test commands are composed; browser failure must stop the lane and remain visible.
One ordinary review covers this tooling slice. Product family design and delivery
planning proceed independently; no new contract writer runs before acceptance.

Delivery investigation distinguishes repository CI, local generated Preview,
durable product upgrade, rollback and public hosting. The user is asked only for
the target environment, without credentials. Planning and local investigation
continue while that operational choice is pending.

### 2026-09-24 Content/Directory decision accepted; Task 1 authorized

Tech Lead `/root/content_directory_decision` proposes experiment ADR-0074,
SHA-256 `4b9b741c074794c1d20da960eca9fb95f795fca4bda6dbc7932e100eb29f5341`.
Independent read-only `/root/directory_decision_review` verified this exact hash,
the six technology decision gates and the actual compiler/store/API/verifier
seams. Verdict: `APPROVED_FOR_STANDING_ACCEPTANCE: yes`, P0/P1/P2 `0/0/0`,
with no material uncertainty. Under the 2026-09-01 founder standing authorization,
PM accepts this exact bounded experiment. This is decision acceptance, not
implementation or product acceptance; no cloud, provider or release is authorized.

The [directory implementation plan](../plans/2026-09-24-content-directory.md)
is authorized within that decision. The frozen family is `content-directory/v1`,
compiler profile `content-directory@1.0.0`, six existing exact core locks,
reader filtered list/detail and curator create/correct/show/hide. ADR FAM/API
clauses freeze fields, limits, query/response shapes, role scope, concurrency,
receipts and errors. Existing public exports and all prior generated bytes remain.

Serialized Task 1 owner `directory_profile_implementation` may write only the new
private `packages/compiler/src/content-directory-contract.ts`, new focused
contract test/fixture support, and additive old-eight capture fixture/receipt.
Capture the current source identity and actual immutable-input outputs before
source implementation; no existing protected snapshot may change. This task
does not modify compiler facade, generated API/store/UI, adapters or verifier.
The selector is private and returns an immutable exact profile, undefined for
unrelated inputs and a safe error for malformed directory-shaped candidates.
Root owns plan/status/ledger and integration. Dependent runtime/UI writers wait
for this task's concrete private interface; no parallel shared-contract edits.

### 2026-09-24 Emitted definition lane accepted locally

`definition_lane_completion` changed only the two regression scripts. The first
direct compiler run found missing generated Prisma setup (95/97); the fixed lane
now uses the existing `@factory/control-plane prisma:generate` before the two
existing numeric/calculated suites. Focused RED/GREEN passes, script tests pass
12/12 and actual emitted tests pass 97/97. Root ran the complete definitions lane:
all eight steps returned zero, including protected compatibility. No generated
output, schema, dependency, database or provider service changed.

Independent read-only `/root/directory_decision_review` approves this separate
ordinary tooling slice, P0/P1/P2 `0/0/0`. Windows/POSIX command order, fail-closed
client generation and emitted tests, provider-variable stripping and safe summary
remain verified. PM accepts the bounded tooling result; controller may commit
...
### 2026-09-24 Directory Task 2 actual PostgreSQL attempt 1

The Task 2 verification paths include the opt-in
`packages/compiler/test/content-directory-postgres.test.ts`. Root supplied a fresh
loopback-only PostgreSQL 16 fixture with an exact random name and ownership labels;
database credentials stayed in ignored local environment files. Runtime emitter
SHA-256 was `e51fdac0af17ce0eb690948a20bfeb112da643863646cfe337dba8a34d521ea7`.

The real database business case passed: literal percent/underscore/backslash
search, hidden list/detail filtering, concurrent same-key creation, concurrent
same-version correction with one winner, injected receipt-write rollback, and
replay after client disconnect/reconnect. This is not a container restart or a
complete generated-product acceptance result. The command still exited 1 because
Windows reported `EBUSY` in temporary-client teardown. Preserve this failed first
attempt; the engineer owns a scoped fixture-lifetime correction.

Root verified labels and removed exact container
`factory-directory-task2-061a497023424adeb237ba9326cfa84b`; no matching running
container remained. Temporary client directory
`C:/Users/15492/AppData/Local/Temp/archeform-directory-postgres-qjJe7U` remains.
Its `node_modules` entry is a junction to this worktree's Control Plane dependencies.
Automatic command approval rejected the subsequent validated native PowerShell
cleanup before execution with `blocked by policy` and no further reason. Do not
retry that deletion through another shell/tool or count scratch cleanup as passed.
No application data volume or shared dependency path was deleted.

### 2026-09-24 Directory Task 2 narrow runtime accepted; admission next

Frozen runtime emitter SHA-256:
`522f85e878ead3ec9840712413b39b5f80b19ef9b2c700174902a207433a98d1`.
Root reran 162/162 focused cases (35 runtime, 50 profile, 77 compatibility),
including emitted API/native adapter typechecking; compiler typecheck and the
nine-package build prerequisite pass. Historical old-eight output bytes remain
equal. The corrected actual PostgreSQL probe passes 1/1 in 9.65 seconds. Its
exact container `factory-directory-task2-87ea309d7a564585b70eafd50b079fa5` was
removed after ownership checks; no Directory test container remains.

The corrected probe disconnects before exit and leaves its isolated client to
the controller. Automatic approval also blocked removal of its workspace path:
`generated/.directory-task2-87ea309d7a564585b70eafd50b079fa5/archeform-directory-postgres-87ea309d7a564585b70eafd50b079fa5`.
Both client paths remain `cleanup_required`; neither blocked deletion is retried
through another tool. Fixture PostgreSQL used tmpfs, with no retained database
volume. Ignored local environment files remain local and are not evidence files.

Independent `/root/directory_decision_review` approves Task 2 source/runtime
integration and dependent implementation with P0/P1/P2 `0/0/0`, explicitly
retaining the scratch-cleanup limitation and withholding complete family/product
acceptance. PM accepts that narrow implementation outcome; no new product count.

Serialized Task 3 owner remains `directory_profile_implementation`. Its write
scope changes to `packages/adapters/src/requirements/definition-family-registry.ts`,
`product-definition-data.ts`, `definitions/product-definitions.v1.json`, existing
catalogue/interpretation selection seams under that requirements directory, and
focused `packages/adapters/test/content-directory-definition.test.ts` plus current
membership tests only where the ninth registered definition requires updates.
Implement the frozen ADR-0074 canonical data and fixed-family admission; preserve
historical lists/snapshots and old-eight generated bytes. Do not add public exports,
packages, UI/runtime/verifier edits or placeholder acceptance bindings. Unsupported
contact/submission/upload/identity requirements must remain explicit. Root owns
docs, integration and Git; the delivery harness retains its disjoint four paths.

### 2026-09-24 Durable-delivery harness pre-run correction

The four-file fixture implementation passes its initial 19 unit cases and fixed
plan CLI. No real rehearsal resources have been created. Root's read-only engine
preflight exposed missing Windows Program Files environment variables needed for
Docker Compose plugin discovery; Engine 29.6.2 and Compose 5.3.1 work directly,
and the narrowly extended OS-only environment restores the read-only command.

Independent review reports P0/P1/P2 `0/1/2`: direct-child-only termination does not
bound a Playwright process tree; readiness accepts late success after its deadline;
and a chunked oversized HTTP body can cause repeated 413 writes and an uncaught
headers-sent error. All have concrete local reproductions. Existing owner
`durable_delivery_implementation` is correcting these same four paths with focused
regression tests, bounded native process-tree supervision and no new dependency.
The actual Docker rehearsal remains on hold until these findings are corrected;
this is a failed pre-run check, not delivered upgrade/rollback evidence. Directory
definition work continues independently under its accepted contract.

### 2026-09-24 Directory data review complete; presentation implementation active

Task 3 adds the ninth registered definition only in the current working tree.
Root confirms 220 focused adapter cases plus 13 Appointment admission cases and
adapter typecheck; engineer confirms 127 compiler profile/compatibility cases.
Independent review proves the historical eight JSON entries remain byte-identical.
One P2 in provider guidance was corrected to the accepted title/summary/body
limits 120/280/12000 with RED 2/40 then GREEN 40/40. Final definition JSON hash is
`f5a856c62a5fe3003b9c4ec47d768eac33f31a68d51cfc4f8b6139832bff2f89`.
The same reviewer closes findings at P0/P1/P2 `0/0/0` and permits dependent work.

The derived executable index correctly reports `definition.case.missing`.
Keep this admission change uncommitted until the actual Directory case restores
the required lane; do not fabricate a binding or waive that failure. Product
acceptance remains eight definitions/four runtime families. Task 3 code review
does not grant product or complete iteration acceptance.

Serialized Task 4 owner `directory_profile_implementation` now owns new
`packages/compiler/src/content-directory-presentation.ts`, new focused
`packages/compiler/test/content-directory-presentation.test.ts`, narrow compiler
`index.ts` integration/styles and test fixture support, plus
`apps/compiler-worker/src/verifier/{verification-graph-plan,verification-profiles,role-journey,probes}.ts`
and their directly affected tests. Adapter admission is frozen except concrete
integration fixes disclosed to root. Preserve the accepted private profile,
all old-eight bytes and public package exports. Use existing registry assets,
approved workspace material and pinned local icons per ADR-0074; no new package,
external imagery, placeholder navigation or duplicated Approval workflow UI.
Verify complete reader search/detail and curator create/correct/show/hide, role
cache/late-response isolation, conflict/retry recovery and real complete-workspace
390/768/1440 layout. Root owns actual Docker execution, evidence, docs and Git.

### 2026-09-24 Durable rehearsal first actual attempt started

The focused recheck closes all three harness findings at P0/P1/P2 `0/0/0`.
Root independently passes 25 unit cases and the real read-only preflight; emitter
SHA-256 is `c601146fc81185bca23efb8816ad52009f161d3044b2b8c470164ccb38b23d8f`.
Root starts the fixed CLI run with ID `f5c6514c05895336ea7a93e5`. Immutable fixture
validation passes and actual image construction starts. Final business and cleanup
outcomes are pending; there is no cloud or production rollout claim.

Attempt 1 exited 1: all five application images were built and captured, then
the unconditional PostgreSQL pull failed; build phase duration was 97,827 ms.
No container, database volume or network was created. Cleanup also failed because
the image inventory returns short ID prefixes while the script compared suffixes.
Root inspected all three run/application/harness labels, exact single tags and
absence of attached containers, then removed exactly those five images. No owned
Docker resources remain. The ignored failed-run source/lease remains for diagnosis.
No upgrade, rollback or restore assertion ran, and this attempt stays failed.

The harness owner is correcting cached-image selection, exact image-ID cleanup
and startup-failure evidence. A focused RED reproduction exposed a separate test
finally bug: an unsuccessful fake cleanup prevented loopback server shutdown.
The owner's attempted process/scratch cleanup was rejected by automatic command
approval with only `blocked by policy`. Test worker PID `24416` and fake run
`d454c0f988ac93445920ca2d` are recorded as `cleanup_required`; no Docker resources
exist for that fake run. Do not retry the blocked actions through another tool.
Fix future test cleanup independently; these residuals are not a passed teardown.

### 2026-09-24 Directory verifier selector seam accepted

Task 4 exposed a real package-boundary gap: the accepted single private selector
could not be imported by the worker through its root-only package surface.
Tech Lead `/root/directory_verifier_decision` proposed the narrow ADR-0074
amendment at SHA-256
`91ad7bc1bf806fafaa4bbfa656be428567138123c632a2219c48b17195728919`.
Independent `/root/directory_decision_review` verified only FAM-007, IMP-002 and
SEA-001..007 changed and returns `APPROVED_FOR_STANDING_ACCEPTANCE: yes`,
P0/P1/P2 `0/0/0`, without a material choice or external authority requirement.
PM accepts under the existing founder standing policy; this hash supersedes the
previous ADR hash for subsequent implementation.

The existing Task 4 owner may expose exactly `selectContentDirectoryProfile`
and readonly `ContentDirectoryProfile` through `@factory/compiler` root and use
that actual selector in worker verification. All original Graph-plus-separate-lock
validation remains required. No caller witness, relative source/dist import,
duplicate predicate, new package export-map key, dependency or other public
export is authorized. This is the sole exception to the earlier no-new-exports
freeze. Root-selector/worker fail-closed tests, dependency-order builds and fixed
old-eight byte checks establish the boundary before integration. Product and
complete iteration acceptance remain pending.

### 2026-09-24 Durable rehearsal attempt 2 failed with scoped cleanup

Reviewed harness hash `cc6f5ce7cafe10773da95c47259b2f2c8d5b3d9a87b24c5722d56b868f83f45c`
passes root 29/29 unit cases and scoped recheck P0/P1/P2 `0/0/0`. Root also verifies
the cached PostgreSQL lookup without permitting a pull. Actual run
`1798403ec468377082ef5661` passes build (67,278 ms), base-image selection (574 ms),
bootstrap A (14,644 ms) and serve-A readiness (2,039 ms), then exits 1 before
failed-candidate, upgrade, rollback or restore phases. Harness cleanup reports
removed; independent owned-label inventory finds no containers, volumes or images.

The CLI's final `failure: null` is a reporting defect: its reporter consumes only
named phase events while initial A business assertions run outside those phases.
The nonzero child exit remains a failed attempt. Buffer-only Playwright attachments
are not retained by the list reporter, leaving the output directory empty. The
owner must persist safe JSON/screenshots, phase the A business actions and retain
safe assertion/status metadata before another attempt. Do not infer the business
failure cause from missing diagnostics or mark this attempt successful. These
fixes do not change generated product behavior or expand deployment authority.

### 2026-09-24 Directory bounded response proof ownership

Task 4 additionally owns `verification-environment.ts` and its focused test.
The existing accepted visibility-proof requirement needs a private Directory
read expectation and a transient boolean result. Only exact-selected Directory
plans may request it. Stream at most 64 KiB (allowing the accepted 12,000-character
plain-text detail with Unicode), retain the existing operation deadline, and
reject malformed or oversized responses without partial success. Use small
verifier-authored fixtures for list proofs. No raw response record leaves this
boundary or enters evidence. Existing non-Directory requests and created-ID
capture retain their behavior. This is a bounded implementation-path extension,
not a new public API or deployment/data authority.

### 2026-09-24 Durable attempt 3 isolates the screenshot assertion

Root confirms 33/33 focused cases and independent scoped review P0/P1/P2 0/0/0
at harness SHA-256 `0573555324ded2a14aad38d205e5245d0557f7612965f1a9bf1967b6c1b41c30`.
Actual run `15a8e326e3b1e87aafc670bd` passes immutable fixtures, build (81,996 ms),
bootstrap, gateway, both creates, correction, transition and history checks. It
fails at `a-capture` (5,072 ms), before any upgrade/recovery assertion. Persisted
safe diagnostics record all seven HTTP checks at their expected 201/200 statuses.
Root visually observes the created and corrected records in the failure image.
The exact-text locator needs emitted-DOM reproduction; this remains a failed run,
not a passed deployment. Safe evidence and the screenshot are preserved under
`docs/acceptance/evidence/local-durable-delivery/attempt-03/`.
Scoped teardown succeeds, and independent run-label inventory confirms no owned
containers, volumes, images or networks. Earlier blocked residuals remain open.

### 2026-09-24 Directory actual-case preparation

With ADR-0074's Graph/API contract frozen, `directory_acceptance_case` owns only
new `e2e/content-directory.spec.ts` and, if needed, new test-local
`e2e/helpers/content-directory.ts`. Reuse existing immutable lifecycle and exact
Preview helpers without editing them. Prepare the executable product case and
its safe evidence; do not create a passing evidence record or case binding before
actual execution. No production source, packages, environment, Docker mutation,
Git or other owned path changes. Coordinate observable UI names with Task 4;
root owns real run, cleanup, visual inspection, index binding and delivery.

Root inspects the first emitted Chromium screenshot batch (mocked HTTP routed
to the actual emitted in-memory runtime, not PostgreSQL). Reader content hierarchy,
plain-text detail and curator form are present, but mobile content starts too low
and category glyphs all reuse a receipt. One grouped visual correction requests
less mobile header/filter whitespace, purposeful distinct knowledge glyphs from
the already pinned local Lucide package, and a primary curator create action.
Any additional SVG stays Directory-private, hash-checked and license-covered;
the old-eight asset outputs cannot change. Early images remain under ignored
`generated/.directory-task4-ui-before-review`. This is the existing acceptance
review's correction pass, with no new dependency or separate visual gate.

### 2026-09-24 Durable local rehearsal accepted

Actual attempt 4 `7dd8d049d3ef0673fa227d1b` exits 0 with `delivery.completed`:
24 passed phases, failed-candidate writable-A retention, healthy B promotion,
new B writes, PostgreSQL restart, retained-image rollback, original-A replay and
separate verified backup restore. All four owned Docker resource inventories are
empty after scoped cleanup. Safe evidence, before/after screenshots and root's
independent resource proof are preserved under
`docs/acceptance/evidence/local-durable-delivery/attempt-04`.

The owner reports 34/34 harness cases; root previously confirmed 33/33 and now
independently passes the new emitted-browser locator regression. The original
exact-text failure is reproduced against actual emitted React/CSS and fixed only
in the test helper. Root visually inspects B/rollback screenshots. Existing reviewer
`directory_decision_review` closes the corrected assertion and actual evidence at
P0/P1/P2 0/0/0 and accepts the narrow task for a bounded commit. Script SHA-256
is `0573555324ded2a14aad38d205e5245d0557f7612965f1a9bf1967b6c1b41c30`;
the acceptance record binds the exact helper/spec and bundle/image identities.

Recorded phase duration is 145,345 ms excluding cleanup/unphased overhead. One
transient denied read appears in readiness polling; this is not uninterrupted
availability. The fixture has zero declared capability effects, so no nonzero
notification delivery is claimed. Earlier policy-blocked residuals remain
`cleanup_required`. PM accepts only the local test experiment, not a production
adapter, hosted deployment, full family or whole-branch release.

Directory Task 4 separately passes root 10 emitted UI and 39 bounded-worker cases.
Root confirms the corrected reader390/curator1440 images address the grouped
