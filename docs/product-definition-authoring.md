...
Its eight registered entries supply canonical requirements/blueprints and selection
...
The read-only
`factory.product-definition-case-index/v1` output from
`scripts/definition-case-index.mjs --check` is derived from the catalogue and
the checked-in bindings in `scripts/definition-case-bindings.mjs`. It records
one case path, runtime family and evidence path for each registered definition.
The index is routing information only: it cannot register a row, promote
acceptance or replace the PM acceptance record. A missing, duplicate,
unregistered or missing-file binding fails before expensive runtime work.
...
Current catalogue assertions use the single eight-entry case binding manifest;
the first seven protected keys retain their historical order. A separate
current-six snapshot freezes exact generated bytes before calculated-total work.
...
## Equipment Procurement recipe

`equipment-procurement-approval` uses the accepted calculated profile without
a product-specific runtime branch. Its admission also completed a shared
calculated-summary spacing repair with multi-record responsive regression.
Declare one equipment-request with required itemName,
positive integer quantity, positive unitPrice, server-owned total and justification.
Use employee/manager/finance and the existing return/correct/resubmit journey.
Both operands can change in one correction. No artificial date or enum is needed.

The reviewed row must explicitly keep unsupported inventory tracking, stock
changes/reservation, supplier execution, payments, private identity, free items,
multiple lines, taxes and rounding as material clarification boundaries. Do not
suggest that an approval action performs those jobs. Preserve the mirrored
canonical/provider guidance and its requirement checksum.

Reuse `e2e/equipment-procurement.spec.ts` and the shared Approval batch helper.
Declare the output, exact summaries and visible-label policy; verify two distinct
items and their calculated totals through both cards and history. Shared numeric
precision/transaction evidence is retained at the capability boundary. Actual
admission status and limitations live in `docs/acceptance/equipment-procurement.md`;
registration alone does not establish product acceptance.
