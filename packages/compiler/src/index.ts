...
function renderPageRuntime(
  ...
  return [
    ...
    approval
      ? "  return <main className='generated-app approval-v1' data-theme={definition.themeMode}><header className='generated-header'><div><h1>{definition.applicationName}</h1><p>Requests and approvals</p></div><div><label htmlFor='demo-role'><ApprovalIcon name='user-round' />Demo role</label><select id='demo-role' value={role} onChange={(event) => setRole(event.target.value)}>{definition.policy.roles.map((candidate) => <option key={candidate} value={candidate}>{candidate}</option>)}</select></div></header><nav aria-label='Application routes'>{projection.navigation.map((item) => <a aria-current={item.route === requestedRoute ? 'page' : undefined} href={item.route} key={item.id}><ApprovalIcon name={item.route === projection.routeFallback.rootRoute ? 'house' : 'receipt-text'} />{item.label}</a>)}</nav>{error ? <p className='generated-error' role='alert'>{error}</p> : null}<section className='generated-page'>{activePage.blocks.map((block) => <BlockRenderer key={block.id} block={block} context={context} />)}</section></main>;"
...
function renderWebStyles(
  ...
  return [
    ...
    ...(profile === "approval-v1"
      ? [
          ".approval-v1 .approval-icon { display: inline-flex !important; flex: 0 0 auto; width: 1.125rem; height: 1.125rem; vertical-align: middle; margin-inline-end: .4rem; } .approval-v1 .approval-icon svg { width: 100%; height: 100%; }",
          ...
          ...(needsApprovalSummaryExtension(graph)
            ? [
                ...
                ".approval-v1 .approval-record-title { margin: 0; min-width: 0; overflow-wrap: anywhere; font-size: 1.125rem; line-height: 1.4; } .approval-v1 .approval-record-title > span { display: block; margin-bottom: .25rem; color: var(--factory-muted); font-size: .8125rem; font-weight: 600; }",
              ]
            : []),
