...
export function renderAppointmentApiDispatch(source: string, profile?: AppointmentRuntimeProfile): string {
  ...
  const context = hasVerifiedIdentity
    ...
    : "function appointmentServerContext(_request: { headers: Record<string, string | string[] | undefined> }, _entity: string, _action: string): { role:string;scope:string;graphHash:string } { throw new Error('Identity policy denied: verified appointment session required.'); }";
