...
export interface AppointmentRuntimeProfile {
  readonly capability: "scheduling.appointment@1.0.1";
...
function plain(
  ...
  if (![Object.prototype, null].includes(Object.getPrototypeOf(object)))
    fail(400, code);
  return object;
}
...
function cancellationReason(value: unknown): string {
  if (typeof value !== "string") fail(400, "appointment.invalid_request");
...
function context(value: unknown): Context {
  ...
  if (
    ...
    candidate.factoryServer !== true
  )
    fail(403, "appointment.unauthorized");
  for (const key of keys.slice(1))
    nonEmpty(candidate[key], "appointment.unauthorized");
...
function version(value: unknown): number {
  if (!Number.isSafeInteger(value) || (value as number) < 0)
    fail(409, "appointment.version_conflict");
...
function canonicalUtc(value: unknown): string {
  ...
  if (!match) fail(400, "appointment.schedule_invalid");
...
function timezone(value: unknown): void {
  const source = nonEmpty(value, "appointment.schedule_invalid");
  if (source.length > 64 || /[\u0000-\u001f\u007f]/.test(source))
    fail(400, "appointment.schedule_invalid");
  try {
...
function slot(
  profile: AppointmentRuntimeProfile,
  schedule: Record<string, unknown>,
) {
  const scheduleTimezone = nonEmpty(
    schedule[profile.fields.scheduleTimezone],
    "appointment.schedule_invalid",
  );
  timezone(scheduleTimezone);
  const startUtc = canonicalUtc(schedule[profile.fields.scheduleStart]);
  const endUtc = canonicalUtc(schedule[profile.fields.scheduleEnd]);
  if (Date.parse(endUtc) <= Date.parse(startUtc))
    fail(400, "appointment.schedule_invalid");
  return {
    scheduleId: nonEmpty(schedule.id, "appointment.schedule_invalid"),
    serviceId: nonEmpty(
      schedule[profile.fields.scheduleService],
      "appointment.schedule_invalid",
    ),
    startUtc,
    endUtc,
...
function record(
  ...
  if (
    ...
    fail(409, "appointment.state_conflict");
  return {
    id,
    status,
    version: version(value.version),
    scheduleId: nonEmpty(
      value[profile.fields.appointmentSchedule],
      "appointment.version_conflict",
    ),
    customerName: nonEmpty(
      value[profile.fields.appointmentCustomer],
      "appointment.version_conflict",
    ),
    notes:
      value[profile.fields.appointmentNotes] === null ||
      value[profile.fields.appointmentNotes] === undefined
        ? null
        : nonEmpty(value[profile.fields.appointmentNotes]),
    cancellationReason:
      value[profile.fields.appointmentCancellationReason] === null ||
      value[profile.fields.appointmentCancellationReason] === undefined
        ? null
        : nonEmpty(value[profile.fields.appointmentCancellationReason]),
  };
}
...
function bindingEntity(value: unknown): string | undefined {
  if (!value || typeof value !== "object" || !("graphSymbol" in value))
    return undefined;
  const symbol = (value as { graphSymbol?: unknown }).graphSymbol;
  const match =
    typeof symbol === "string"
      ? /^graph\.domain\.([a-z][a-z0-9-]*)$/.exec(symbol)
      : undefined;
  return match?.[1];
}
...
function bindingField(value: unknown, owner: string): string | undefined {
  ...
  return field.graphSymbol === `graph.domain.${owner}` &&
...
export function selectAppointmentRuntimeProfile(
  ...
  const selection = compositionLock.packages.find(
    ({ lock }) =>
      lock.key === "scheduling.appointment" && lock.version === "1.0.1",
  );
  if (!selection) return undefined;
  if (compositionLock.applicationGraphChecksum !== hashApplicationGraph(graph))
    throw new Error(
      "Appointment composition lock does not match the Published Graph.",
    );
  if (
    graph.integration.compositionSelections &&
    !isDeepStrictEqual(
      [...graph.integration.compositionSelections].sort((left, right) =>
        left.lock.key.localeCompare(right.lock.key),
      ),
      [...compositionLock.packages].sort((left, right) =>
        left.lock.key.localeCompare(right.lock.key),
      ),
    )
  )
  ...
  if (
    ...
    !scheduleEntity ||
    !appointmentEntity ||
    new Set([serviceEntity, scheduleEntity, appointmentEntity]).size !== 3
  )
    throw new Error("Appointment capability bindings are invalid.");
  const field = (
    key: Exclude<
      (typeof bindingNames)[number],
      "serviceEntity" | "scheduleEntity" | "appointmentEntity"
    >,
    owner: string,
  ) => {
  ...
  const fields = {
    ...
    appointmentSchedule: field(
      "appointmentScheduleReferenceField",
      appointmentEntity,
    ),
    appointmentCustomer: field(
      "appointmentCustomerNameField",
      appointmentEntity,
    ),
    appointmentNotes: field("appointmentNotesField", appointmentEntity),
    appointmentCancellationReason: field(
      "appointmentCancellationReasonField",
      appointmentEntity,
    ),
  ...
  const entityFields = new Map(
    graph.domain.entities.map((entity) => [
      entity.key,
      new Set(entity.fields.map((item) => item.key)),
    ]),
  );
  if (
    ![serviceEntity, scheduleEntity, appointmentEntity].every((entity) =>
      entityFields.has(entity),
    ) ||
  ...
  return {
    ...
    scheduleEntity,
    appointmentEntity,
    graphHash: compositionLock.applicationGraphChecksum,
    fields,
  };
}

export function createAppointmentCommandRuntime(
  profile: AppointmentRuntimeProfile,
) {
  async function scheduleForSnapshot(
    store: AppointmentCommandStore,
    scheduleId: string,
  ) {
    const schedule = await store.find(
      profile.scheduleEntity,
      nonEmpty(scheduleId),
    );
    if (!schedule) fail(400, "appointment.schedule_invalid");
    const snapshot = slot(profile, schedule);
    if (!(await store.find(profile.serviceEntity, snapshot.serviceId)))
      fail(400, "appointment.schedule_invalid");
    return schedule;
  }
  ...
  async function scheduleForClaim(
    ...
    if (schedule[profile.fields.scheduleStatus] !== "open")
      fail(409, "appointment.schedule_closed");
    ...
    const service = await store.find(
    ...
    if (!service || service[profile.fields.serviceActive] !== true)
      fail(409, "appointment.schedule_invalid");
    const occupancy = (await store.list(profile.appointmentEntity)).filter(
      (item) =>
        item.id !== excluded &&
        item[profile.fields.appointmentSchedule] === schedule.id &&
        active(item[profile.fields.appointmentStatus]),
    ).length;
    if (occupancy >= declaredCapacity)
      fail(409, "appointment.capacity_conflict");
    return schedule;
  }
  ...
  async function commit(
    ...
    mutate: (transaction: AppointmentCommandStore) => Promise<{
      record: AppointmentRecord;
      history: Record<string, unknown>;
      capabilityOperation: "claim" | "confirm" | "move" | "release";
    }>,
  ): Promise<AppointmentRecord> {
    const prior = await store.getAppointmentReceipt(
      input.scope,
      input.idempotencyKey,
    );
    if (prior) {
      if (prior.requestHash !== input.requestHash)
        fail(409, "appointment.idempotency_conflict");
      return record(
        profile,
        plain(prior.response, "appointment.idempotency_conflict"),
      );
    }
    return store.inTransaction(async (transaction) => {
      const replay = await transaction.getAppointmentReceipt(
        input.scope,
        input.idempotencyKey,
      );
      if (replay) {
        ...
        return record(
          profile,
          plain(replay.response, "appointment.idempotency_conflict"),
        );
      }
      const result = await mutate(transaction);
      await transaction.appendAppointmentHistory({
        apiVersion: appointmentMutationContract.history,
        ...result.history,
        appointmentId: result.record.id,
        at: input.now,
        actorRole: input.role,
      });
      await transaction.appendCapabilityEvent({
        actor: input.role,
        capability: profile.effect,
        operation: result.capabilityOperation,
        entity: profile.appointmentEntity,
        recordId: result.record.id,
        outcome: "completed",
        at: input.now,
      });
      await transaction.saveAppointmentReceipt({
        scope: input.scope,
  ...
  return {
    async request(
      ...
      if (
        ...
        !Object.hasOwn(input, "scheduleId") ||
        !Object.hasOwn(input, "customerName")
      )
        fail(400, "appointment.invalid_request");
      if (
      ...
      return commit(store, actor, "request", async (transaction) => {
        const selected = await scheduleForClaim(
          transaction,
          nonEmpty(input.scheduleId),
        );
        ...
        return {
          ...
          history: {
            ...
            toStatus: "requested",
            fromSlot: null,
            toSlot: slot(profile, selected),
            cancellationReason: null,
          },
          capabilityOperation: "claim",
        };
      });
    },
    async confirm(
      ...
      return commit(store, actor, "confirm", async (transaction) => {
        const current = await transaction.find(
          ...
          nonEmpty(input.appointmentId),
        );
        if (!current) fail(404, "appointment.not_found");
        const prior = record(profile, current);
        if (prior.version !== version(input.expectedVersion))
          fail(409, "appointment.version_conflict");
        if (prior.status !== "requested")
          fail(409, "appointment.state_conflict");
        const selected = await scheduleForSnapshot(
          transaction,
          prior.scheduleId,
        );
        const unchangedSlot = slot(profile, selected);
        const changed = await transaction.conditionalUpdate(
          profile.appointmentEntity,
          prior.id,
          prior.version,
          {
            [profile.fields.appointmentStatus]: "confirmed",
        ...
        return {
          ...
          history: {
            ...
            toStatus: "confirmed",
            fromSlot: unchangedSlot,
            toSlot: unchangedSlot,
            cancellationReason: null,
          },
          capabilityOperation: "confirm",
        };
      });
    },
    async reschedule(
      ...
      return commit(store, actor, "reschedule", async (transaction) => {
        ...
        if (nextSchedule === prior.scheduleId)
          fail(400, "appointment.schedule_invalid");
        const previous = await scheduleForSnapshot(
          transaction,
          prior.scheduleId,
        );
        const target = await scheduleForClaim(
          transaction,
          nextSchedule,
          prior.id,
        );
        const changed = await transaction.conditionalUpdate(
          ...
          prior.version,
          {
            [profile.fields.appointmentSchedule]: nextSchedule,
            [profile.fields.appointmentStatus]: "requested",
            version: prior.version + 1,
          },
        );
        ...
        return {
          ...
          history: {
            ...
            toStatus: "requested",
            fromSlot: slot(profile, previous),
            toSlot: slot(profile, target),
            cancellationReason: null,
          },
    ...
    async cancel(
      ...
      const actor = context(contextInput);
      role(actor, ["customer", "staff", "administrator"]);
      const input = exactInput(body, [
        "appointmentId",
        "expectedVersion",
        "cancellationReason",
      ...
      return commit(store, actor, "cancel", async (transaction) => {
        ...
        const reason = cancellationReason(input.cancellationReason);
        const previous = await scheduleForSnapshot(
          transaction,
          prior.scheduleId,
        );
        const changed = await transaction.conditionalUpdate(
          ...
          prior.version,
          {
            [profile.fields.appointmentStatus]: "cancelled",
            [profile.fields.appointmentCancellationReason]: reason,
            version: prior.version + 1,
          },
        );
        if (!changed) fail(409, "appointment.version_conflict");
...
export function renderAppointmentMutationRuntime(
  ...
  source = replaceRendererAnchor(
    ...
    "  private readonly appointmentReceipts = new Map<string, AppointmentMutationReceipt>();\n  private readonly appointmentHistory: AppointmentHistoryEntry[] = [];\n  private readonly auditEvents: AuditEvent[] = [];",
  );
  source = replaceRendererAnchor(
    source,
...
export function renderAppointmentApiDispatch(
  source: string,
  profile?: AppointmentRuntimeProfile,
): string {
  if (!profile) return source;
  ...
  source = source.replace(
    ...
    'import { Body, Controller, Get, HttpCode, HttpException, HttpStatus, Module, Param, Post, Req } from "@nestjs/common";',
  );
  const graphHash = profile.graphHash ?? "locked-appointment-graph";
  const hasVerifiedIdentity = source.includes(
    "export function resolvePrincipalContext(",
  );
  const context = hasVerifiedIdentity
    ? `function appointmentServerContext(request: { headers: Record<string, string | string[] | undefined> }, entity: string, action: string) { try { const principal = resolvePrincipalContext(request); return { role: roleFrom(request, entity, action), scope: 'fixture:' + principal.sessionId, graphHash: ${JSON.stringify(graphHash)} }; } catch { throw new AppointmentDomainError(403, 'appointment.forbidden'); } }`
    : "function appointmentServerContext(_request: { headers: Record<string, string | string[] | undefined> }, _entity: string, _action: string): { role:string;scope:string;graphHash:string } { throw new AppointmentDomainError(403, 'appointment.forbidden'); }";
  ...
  return source
    ...
    .replace(
      ...
      `class GeneratedController {${route}`,
    )
    .replace(
    ...
    .replace(
      ...
      `if(entity===${JSON.stringify(profile.appointmentEntity)})return await applicationRuntime.appointmentCommand(appointmentServerContext(request,entity,'create'),entity,undefined,'create',request.headers['x-factory-idempotency-key'],body); return await applicationRuntime.create(`,
    )
    .replace(
      "return await applicationRuntime.transition(",
      `if(entity===${JSON.stringify(profile.appointmentEntity)}&&['confirm','reschedule','cancel'].includes(event))return await applicationRuntime.appointmentCommand(appointmentServerContext(request,entity,event),entity,recordId,event,request.headers['x-factory-idempotency-key'],body); return await applicationRuntime.transition(`,
    );
}
