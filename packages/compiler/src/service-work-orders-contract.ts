...
export interface ServiceWorkOrdersProfile {
  readonly key: "service-work-orders";
  ...
  readonly orderVersionMaximum: 2147483647;
  readonly titleMaximum: 160;
  readonly serviceLocationMaximum: 160;
  readonly descriptionMaximum: 2000;
  readonly reasonMaximum: 500;
  readonly resolutionNoteMaximum: 2000;
}
const locks = [
  [
    "core.crud",
...
function reject(): never {
  throw new Error("Unsupported Service Work Orders profile.");
}
...
/** Distinct assignment/history coordinates or new verbs identify near-matches; labels never do. */
function candidate(graph: unknown): boolean {
  ...
  if (
    entities.some((entity) => {
      ...
      return (
        (fields.has("serviceLocation") && fields.has("assigneePrincipalId")) ||
        ((fields.has("workOrderId") || fields.has("workOrder")) &&
          (fields.has("orderVersion") || fields.has("actorPrincipalId"))) ||
        (fields.has("orderVersion") && fields.has("actorPrincipalId"))
      );
    })
  )
    return true;
  const verbs = ["assign", "reassign", "resolve"];
  return (
...
export function selectServiceWorkOrdersProfile(
  graph: ApplicationGraphV1,
  compositionLock?: CapabilityCompositionLockV1,
): ServiceWorkOrdersProfile | undefined {
  try {
    ...
    > = {
      "core.crud": {
        entityKey: symbol("domain", orderEntity),
      ...
      "core.identity-policy": {
        ...
        defaultRole: symbol("policy", dispatcher),
        authenticatedRole: symbol("policy", technician),
      },
      // Existing owner-aware composer chooses the first actor when there is no approval decision.
      "core.audit": { actorRole: symbol("policy", dispatcher) },
      "core.notification": { recipientRole: symbol("policy", dispatcher) },
      "core.policy-declarations": {},
    };
    ...
    return Object.freeze({
      ...
      titleMaximum: 160,
      serviceLocationMaximum: 160,
      descriptionMaximum: 2000,
      reasonMaximum: 500,
      resolutionNoteMaximum: 2000,
    });
  } catch {
    return reject();
  }
