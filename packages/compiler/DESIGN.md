...
## Approved baseline and active business extension

The founder approved the actual expressive UI delivered at `92f21089`. The
active ADR-0060/0062 correction iteration preserves it while adding prefilled
same-record editing, a required Return reason and safe retry/conflict feedback.
Only an exactly selected correction Graph uses workspace `2.1.0`, components
`1.1.0` and assets `1.1.0`. Old immutable Graphs retain the legacy profile and
exact generated bytes. This section specifies the current implementation target;
completion is tracked in `docs/acceptance/approval-correction.md`.

Return and Save remain labelled because they change business data. Refresh and
navigation retain compact accessible icons. A return explanation appears before
Edit, while earlier decisions remain in readable history after resubmission.
Current-state progress resets to Draft after revision; it must not invent an
audit trail. Correction fields, error states and pending controls inherit the
approved palette, geometry, images and touch targets.
