"""Contract checks for the ui.my-requests asset package."""
...
class MyRequestsPackageTests(unittest.TestCase):
    ...
    def test_package_is_a_candidate_requests_feature_with_valid_fixture(self) -> None:
        package = Path(__file__).resolve().parents[1]
        manifest = validate_component_package(
            package,
available_identities={("ui.my-requests", "2.2.0"), ("ui.app-shell", "2.2.0")},
            approved_package_root=ROOT / "packages" / "components",
        )
        fixture = json.loads((package / "fixtures" / "valid-input.json").read_text(encoding="utf-8"))

        self.assertEqual("frontend/features/my-requests", manifest["output_slots"][0])
