...
class ProfilePagePackageTests(unittest.TestCase):
    def test_template_has_no_nonfunctional_profile_edit_control(self) -> None:
        ...
        self.assertIn("Local preview", template)

    def test_package_is_a_golden_profile_route_with_valid_fixture(self) -> None:
        package = Path(__file__).resolve().parents[1]
        manifest = validate_component_package(
            package,
            available_identities={("ui.profile-page", "2.1.0"), ("ui.app-shell", "2.1.0")},
