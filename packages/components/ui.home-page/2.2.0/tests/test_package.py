"""Contract checks for the ui.home-page asset package."""
...
class HomePagePackageTests(unittest.TestCase):
    ...
    def test_package_is_a_candidate_home_route_with_valid_fixture(self) -> None:
        package = Path(__file__).resolve().parents[1]
        manifest = validate_component_package(
            package,
available_identities={("ui.home-page", "2.2.0"), ("ui.app-shell", "2.2.0")},
            approved_package_root=ROOT / "packages" / "components",
        )
        fixture = json.loads((package / "fixtures" / "valid-input.json").read_text(encoding="utf-8"))

        self.assertEqual(["ui.app-shell"], [item["key"] for item in manifest["requires"]])
