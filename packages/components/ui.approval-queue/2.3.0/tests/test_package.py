"""Contract checks for the ui.approval-queue asset package."""
...
class ApprovalQueuePackageTests(unittest.TestCase):
    ...
    def test_package_is_a_candidate_queue_feature_with_valid_fixture(self) -> None:
        package = Path(__file__).resolve().parents[1]
        manifest = validate_component_package(
            package,
available_identities={("ui.approval-queue", "2.3.0"), ("ui.app-shell", "2.3.0")},
            approved_package_root=ROOT / "packages" / "components",
        )
        fixture = json.loads((package / "fixtures" / "valid-input.json").read_text(encoding="utf-8"))

        self.assertEqual("frontend/features/approval-queue", manifest["output_slots"][0])
        self.assertEqual([], list(Draft202012Validator(manifest["input_schema"]).iter_errors(fixture)))

    def test_pending_decision_disables_only_the_affected_queue_actions(self) -> None:
        package = Path(__file__).resolve().parents[1]
        template = (package / "templates" / "ApprovalQueue.tsx").read_text(encoding="utf-8")

        self.assertIn("pendingDecisionId?: string", template)
        self.assertIn('disabled={pendingDecisionId === request.id}', template)
        self.assertIn('aria-busy={pendingDecisionId === request.id}', template)
