...
import {
  ...
  customerRequestsBlueprint,
} from "../../compiler/test/fixtures/customer-requests.js";
describe("Customer Requests exact composition", () => {
  it("uses six locks, four pages, no fake history and one unique history version index", () => {
    const { graph, compositionLock } = customerRequestsInput();
    expect(compositionLock.packages).toHaveLength(6);
    expect(graph.page.pages).toHaveLength(4);
    expect(graph.domain.seedData).toEqual([]);
    expect(graph.domain.entities[0]!.indexes).toEqual([
      { fields: ["status"] },
      { fields: ["customerPrincipalId"] },
    ]);
    ...
    expect(graph.flow.flows[0]!.transitions).toHaveLength(3);
    expect(
  ...
  it("offers only the exact six-package plan and rejects missing/extra keys", () => {
    ...
    expect(plans[0]!.key).toBe("standard");
