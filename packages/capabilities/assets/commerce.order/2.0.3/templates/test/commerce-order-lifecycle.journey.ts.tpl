...
describe("commerce order lifecycle", () => {
  it("authorizes a persisted initial order before preparing a transition", async () => {
    const draft = await commerceOrderCreateHandler.create(
      ...
      { authorizer: { assertCreateAllowed: async () => undefined }, store: { createInitial: async () => ({ id: "server-1", status: "draft", version: 0 }) } },
    );
    expect(draft).toEqual({ id: "server-1", status: "draft", version: 0 });
    expect(commerceOrderTransactionOperationAdapter.prepare(commerceOrderTransactionOperationAdapter.parseRequest({ orderId: draft.id, expectedVersion: draft.version, transition: "submit", idempotencyKey: "submit-1", payloadDigest: "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" })).command.aggregate.entity).toBe("{{orderEntity}}");
  });
});
