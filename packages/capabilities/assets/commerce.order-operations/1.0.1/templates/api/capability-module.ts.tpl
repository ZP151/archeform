  ...
  switch (command.command) {
    ...
    case "record-partial-payment": {
      ...
      if (amount >= outstanding) {
        throw new Error("Commerce order partial payment must be below the outstanding amount.");
      }
      return createPlan(
        "payment-pending",
        "capture-partial",
    ...
    case "capture-payment": {
      ...
      const amount = commandAmount(command);
      if (amount > outstanding) {
        throw new Error("Commerce order payment exceeds the outstanding amount.");
      }
      if (amount !== outstanding) {
        throw new Error("Commerce order final payment must match the outstanding amount.");
      }
