...
export interface TransactionDependenciesV1 {
  readonly transactionExecutor: CommerceTransactionExecutorV1;
}

export interface TransactionOperationAdapterV1<Request, Context, Response> {
  parseRequest(request: unknown): Request;
  prepare(request: Request): {
    readonly command: CommerceTransactionCommandV1;
    readonly context: Context;
  };
  createStore(
    context: Context,
    dependencies: TransactionDependenciesV1,
  ): CommerceTransactionStoreV1;
  present(
    result: CommerceTransactionResultV1,
    context: Context,
  ): Response;
}
